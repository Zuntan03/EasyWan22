...
`タスクマネージャー` の `パフォーマンス` タブの `アクティブな時間` は読み込みも影響しますので、SSD の寿命を心配する観点では不適切です。  
ComfyUI を起動して最初の生成ではモデルのページファイルへの書き込みが発生しますが、以降の生成ではモデルへの変更がなければページファイルへの書き込みは発生しません。  
数回の生成や SSD のアクティブな時間ではなく、一週間程度の実際の利用状況を元に判断してください。
